...
type UserLogin = Parameters<typeof trpc.users.signup.mutate>[0];
...
export async function loginNewUser(
  ...
  try {
    const user = await trpc.users.signup.mutate(userLogin);

    if (isAdmin) {
      await trpc.users.makeAdmin.mutate(user.id);
    }
  } catch (error) {
...
export async function asUser<T extends any>(
  ...
  const [user] = await Promise.all([
    loginNewUser(page, userLogin, isAdmin),
    (async () => {
